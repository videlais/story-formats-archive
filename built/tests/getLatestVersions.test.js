...
describe('getLatestVersions without story-formats directory', () => {
    ...
    it('should only process formats present in the formats array', async () => {
        axios.get.mockResolvedValue({ data: Buffer.from('test') });
        const db = {
            'format1': [{
                    version: '1.0.0',
                    files: ['file1.js'],
        ...
        expect(axios.get).toHaveBeenCalledWith(`${paths.base_URL}/format2/1.0.0/file2.js`, { responseType: 'arraybuffer' });
        expect(axios.get).toHaveBeenCalledTimes(1);
    });
    it('should use semver to select the latest version', async () => {
        ...
        expect(axios.get).toHaveBeenCalledWith(`${paths.base_URL}/format1/2.0.0/file2.js`, { responseType: 'arraybuffer' });
    });
    it('should create directories for each format and version', async () => {
        const mkdirSpy = jest.spyOn(fs, 'mkdirSync').mockImplementation((path) => path.toString());
