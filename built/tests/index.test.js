...
describe('CLI commands via program.parseAsync', () => {
    ...
    describe('latest command', () => {
        it('should download all formats when no format names are given', async () => {
            ...
            expect(getLatestVersionsMock).toHaveBeenCalledWith(expect.objectContaining({ chapbook: expect.anything() }), ['chapbook'], expect.any(Object));
        });
        it('should download only the specified formats', async () => {
            const getLatestVersionsMock = jest.spyOn(await import('../src/getLatestVersions.js'), 'getLatestVersions').mockResolvedValue(undefined);
            setupProgram();
            await program.parseAsync(['node', 'sfa-get', 'latest', 'chapbook']);
            expect(getLatestVersionsMock).toHaveBeenCalledWith(expect.any(Object), ['chapbook'], expect.any(Object));
        });
        it('should handle a database error and exit with code 1', async () => {
            mockGetJSONDatabase.mockRejectedValue(new Error('network error'));
            setupProgram();
    ...
    describe('get command', () => {
        it('should download a specific format version', async () => {
            ...
            expect(getSpecificVersionMock).toHaveBeenCalledWith(expect.any(Object), 'chapbook', '1.0.0', expect.any(Object));
        });
        ...
        it('should print an error and exit when the format is not in the database', async () => {
            ...
            expect(console.error).toHaveBeenCalledWith('❌ Story format "unknown-format" not found in the database.');
            expect(process.exit).toHaveBeenCalledWith(1);
        });
        it('should handle a database error and exit with code 1', async () => {
            mockGetJSONDatabase.mockRejectedValue(new Error('network error'));
    ...
    describe('verify command', () => {
        let mockVerifyFormatVersion;
        let mockVerifyAllInstalledFiles;
        let mockPrintVerificationResults;
        beforeEach(async () => {
            const verifyModule = await import('../src/verifyFiles.js');
            mockVerifyFormatVersion = jest.spyOn(verifyModule, 'verifyFormatVersion').mockReturnValue([]);
            mockVerifyAllInstalledFiles = jest.spyOn(verifyModule, 'verifyAllInstalledFiles').mockReturnValue([{ format: 'chapbook', version: '1.0.0', file: 'format.js', status: 'valid', path: './story-formats/chapbook/1.0.0/format.js' }]);
            mockPrintVerificationResults = jest.spyOn(verifyModule, 'printVerificationResults').mockImplementation(() => { });
        });
