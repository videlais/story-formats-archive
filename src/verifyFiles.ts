...
export interface VerificationResult {
    ...
    version: string;
    file: string;
    status: 'valid' | 'invalid' | 'missing' | 'no-checksum';
 ...
 */
function verifyFile(filePath: string, expectedChecksum: string): { valid: boolean; actualChecksum: string } {
 ...
 */
export function verifyFormatVersion(
 ...
 */
export function verifyAllInstalledFiles(
    filteredDB: FilteredDatabase,
    storyFormatsDir = './story-formats'
): VerificationResult[] {
    const results: VerificationResult[] = [];
 ...
 */
function getInstalledFormats(storyFormatsDir = './story-formats'): Array<{ format: string; version: string }> {
    const installations: Array<{ format: string; version: string }> = [];
    ...
    try {
        const formatDirs = readdirSync(storyFormatsDir).filter((item: string) => {
        ...
        for (const formatDir of formatDirs) {
            ...
            try {
                const versionDirs = readdirSync(formatPath).filter((item: string) => {
